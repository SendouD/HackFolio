...
function EditHackathonDetails(props) {
  ...
  function validateForm() {
    ...
    if (
      !["online", "offline", "hybrid"].includes(
        formData.eventMode.toLowerCase()
      )
    )
      newErrors.eventMode = "Invalid Event Mode";
    if (!/^[1-9]$|^10$/.test(formData.teamSize))
      newErrors.teamSize = "Team size must be between 1-10";
    if (!/^[A-Za-z][A-Za-z\s]*$/.test(formData.partProf))
      newErrors.partProf = "Invalid Profile";
    if (!/^[A-Za-z][A-Za-z\s]*$/.test(formData.prizesDesc))
      newErrors.prizesDesc = "Invalid Prize Description";
    if (isNaN(Date.parse(formData.fromDate)))
      newErrors.fromDate = "Invalid Date";
    if (isNaN(Date.parse(formData.toDate))) newErrors.toDate = "Invalid Date";
    formData.contactLinks.forEach((link, i) => {
      if (!urlRegex.test(link)) newErrors[`contactLinks-${i}`] = "Invalid URL";
    });
      ...
      <div className="mt-10">
        ...
        <div className="flex items-center">
          ...
          <button
            ...
            className="bg-[#5f3abd] font-medium hover:bg-[#5333a5] text-white py-2 px-4 rounded focus:outline-none focus:shadow-outline ml-2 edit-btn"
          >
  ...
  function removeLink(index) {
    const newArr = [
      ...formData.contactLinks.slice(0, index),
      ...formData.contactLinks.slice(index + 1),
    ];
    ...
    <div className="">
      ...
      <div className="">
        ...
        <div className="mt-10">
          ...
          {formData.contactLinks.map((link, i) => (
            <div key={i} className="flex items-center">
              ...
              <button
                ...
                className="remove-link-btn font-medium edit-btn mt-2 mr-0 ml-2"
              >
                Remove
              </button>
            </div>
          ...
          <button
            onClick={addLink}
            className="add-link-btn font-medium edit-btn py-2 px-3"
          >
            Add
          </button>
          <button
            onClick={() => handleEdit("contactLinks")}
            className="bg-[#5f3abd] hover:bg-[#5f3abd] text-white font-medium py-2 px-4 rounded focus:outline-none focus:shadow-outline edit-btn"
          >
        ...
        <button
          ...
          className="edit-inp w-auto bg-[#5f3abd] hover:bg-[#5f3abd] text-white font-medium py-3 px-6 rounded focus:outline-none focus:shadow-outline edit-btn mt-4"
        >
          Save Changes
        </button>
      </div>
