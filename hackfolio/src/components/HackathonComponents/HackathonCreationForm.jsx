...
function HackathonCreationForm(props) {
    ...
    const nameRegex = /^[a-zA-Za-z] [A-Za-z0-9-\s-]{2,19}$/; // Starts with a letter, allows letters, numbers, and hyphens, 3-20 chars long.
    ...
    function validateInputs() {
        ...
        if (!nameRegex.test(hackName)) {
            newErrors.hackName = "Hackathon name must start with a letter and be 3-20 characters long.";
            isValid = false;
        }
        if (!nameRegex.test(uniName)) {
            newErrors.uniName = "University name must start with a letter and be 3-20 characters long.";
            isValid = false;
        }
    ...
    return (
        <div className="hack-form ">
            <div className="form-container w-full max-w-md p-6 rounded-lg shadow-lg bg-white">
                ...
                />
                {errors.hackName && <p className="text-red-500 text-xs mt-1">{errors.hackName}</p>}
                ...
                />
                {errors.uniName && <p className="text-red-500 text-xs mt-1">{errors.uniName}</p>}
